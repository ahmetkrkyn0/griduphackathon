...
| 7 | **Kullanıcı / Operasyon Deneyimi** | `frontend/src/pages/` · `docs/16-ux-tasarim.md` | Gövde metni kontrastı **13,33:1** (`theme.css` `--ink #202b34` / `--bg #f5f6f8`; WCAG AAA eşiği 7:1) — sayı artık dokümanda değil `frontend/src/theme.test.ts`'te yaşar. **Arayüz test altındadır ve sayılar ölçülmüştür:** 162 vitest testi (19 dosya, 4'ü gerçek DOM'da bileşen çizen), artı `frontend/e2e/smoke.spec.ts` ile 7 ekran + NotFound gezilip **canlı API'de 0 konsol hatası / uyarısı** ölçülür (tek tarayıcı, tek genişlik, yalnızca sayfa açılışı — kapsam bu kadardır). Aynı spec `@axe-core/playwright` ile tarar ve **ihlalleri gizlemez: örnek veri kipinde 83 kontrast ihlali** ölçülmüştür (20 Eylül; hepsi `color-contrast`, başka hiçbir WCAG 2.1 AA kuralı ihlal edilmiyor). Bu sayı 19 Eylül'de **5**'ti; aradaki fark `main` birleşmesiyle gelen yeni arayüz katmanından doğan bir **gerilemedir**, saklanmıyor ve `docs/16` §4'te kalem kalem sayılıdır (taban tokenlar bozulmadı — gövde metni hâlâ 13,33:1). Kontrast **dışı** ihlal sayısı her iki kipte de **0'a kilitlidir**; kontrast sayısı yalnızca örnek veri kipinde kilitlenir — canlı kipte ekrandaki satır sayısı veritabanına bağlı olduğu için ölçülüp **raporlanır**, kilitlenmez. **Planlanan 9 ekranın 7'si çalışıyor** (`docs/16` §2): Filo, Tekil Pano (içinde 3D termal ikiz sekmesi), Alarm Konsolu, Olay Analizi / Kara Kutu, Trend & Korelasyon, Cihaz Sağlığı, Bölge Haritası. Kapsam dışı bırakılanlar: Ayarlar ekranı ve mobil PWA. Operasyon döngüsü kapalıdır: uyarı → **Neden / Ne doğrulanmalı / Ne yapmalı / Ne kadar acil** → Onayla/Rafa al → denetim izine kaydı. Sesli ikaz, CSV vardiya raporu, 96 ilçe coğrafi harita. |
...
# Frontend birim + bileşen testleri ve TypeScript tip doğrulaması — 162 test, 0 hata
