...
| 6 | **Ölçeklenebilirlik** | `docs/09-olceklenebilirlik.md` §4–§6 · `loadtest/fleet.py` · `backend/app/api/insights.py` | 1.000 panoda görünme p95 **657 ms**, kayıp **0** (`docs/09` §3'teki düzenek: i7-14700KF / 28 iş parçacığı). **20 Eylül'de altı yapılandırmanın altısı da yeniden koşuldu ve kanıt dosyaları artık klonda commit'li** — ama daha küçük bir makinede (i5-11300H / 8 iş parçacığı). Sonuç: **doymamış rejimde sayılar birebir tuttu** (1.000 panoda görünme p50 385 → 385,5 ms), doyma noktası ise çekirdek sayısıyla orantılı olarak öne geldi. Ölçüm önce yanlışlıkla "gerileme" diye okundu; iki hata (düzeneğin doğrulanmaması ve `written_per_s.p50`'nin tavan sanılması) `docs/09` §4.1c'de **açıkça düzeltildi**. Yapısal eksik kapatıldı: `loadtest/fleet.py` artık koşum makinesini esere yazıyor (`makine` bloğu), yani iki koşum bir daha sessizce farklı donanımda karşılaştırılamaz. **Ama klondaki eserlerin hiçbiri bu bloğu taşımaz** — hepsi bu değişiklikten önce üretildi; onlar için düzenek yalnızca `docs/09` §2/§4.1b'de düzyazı olarak kayıtlıdır (i7-14700KF / Docker Desktop WSL2, yük üreteci **aynı makinede** — yani sonuçlar temkinli). **657 ms, eseri klonda olmayan 13 Eylül koşumundan gelir**; commit'li 1.000 pano eserlerinde görünme p95 710,5 / 754,5 / 763,2 ms'dir (daha küçük makine, `docs/09` §4.1b). Toplu filo durumu ucu `GET /api/v1/fleet/health` (`backend/app/api/insights.py`) + **sayfalanmış** listeler ile 1.000 panel izlenebilir (liste sanallaştırması kullanılmadı). TimescaleDB sıkıştırması **46–48×** (`deploy/initdb/005_compression.sql`; segmentby `pano_id, tag` — oran 1 günden eski parçalarda ölçülür). **F-36:** uyarlanabilir yayın ile mesaj sayısı 86.400 → 50.932 (**%41,0 bastırma**), pano başına aylık **1.071 MB → 632 MB**; **tespit 10 s'de kaldı**, arıza rejiminde dokuz alarm kodunun tamamı aynı turda çıktı. **Uyarı — bu eser 18 Eylül'de üretildi ve bugünkü koddan birebir yeniden üretilmiyor:** aynı komut bugün 50.612 mesaj / %41,4 / 628,3 MB veriyor (~%0,6 sapma), çünkü üreteç 19 Eylül'de değişti (S10–S13). Betik belirlenimlidir (aynı parametrelerle iki koşum birebir aynı); **bayat olan eserdir** ve `--check` ailesi eseri koda karşı denetlemez. |
| 7 | **Kullanıcı / Operasyon Deneyimi** | `frontend/src/pages/` · `docs/16-ux-tasarim.md` | Gövde metni kontrastı: `theme.css` çiftinde **13,33:1**, ekranda gerçekten kazanan `industrial.css` çiftinde (`#182c44` / `#eef2f6`) **12,59:1** — ikisi de WCAG AAA eşiği 7:1'in üstünde ve ikisi de `frontend/src/theme.test.ts`'te kilitli. İki sayının farklı olmasının sebebi sanayi kabuğunun paleti ezmesidir; bunu saklamak yerine ölçüp kilitledik. **Arayüz test altındadır ve sayılar ölçülmüştür:** 166 vitest testi (19 dosya, 4'ü gerçek DOM'da bileşen çizen), artı `frontend/e2e/smoke.spec.ts` ile 7 ekran + NotFound gezilip **canlı API'de 0 konsol hatası / uyarısı** ölçülür (tek tarayıcı, tek genişlik, yalnızca sayfa açılışı — kapsam bu kadardır). Aynı spec `@axe-core/playwright` ile tarar ve **ihlalleri gizlemez: örnek veri kipinde 0 ihlal** ölçülmüştür (20 Eylül; kontrast dâhil hiçbir WCAG 2.1 AA kuralı ihlal edilmiyor) ve sayı testte kilitlidir. **Bu sayı 19 Eylül'de 5, 20 Eylül sabahı 83'tü** — aradaki artış `main` birleşmesiyle gelen sanayi kabuğunun `--dim` token'ını ezmesinden doğan bir **gerilemeydi** ve 83'ün 69'u tek bir renkten geliyordu; dördü de ölçülerek düzeltildi (`docs/16` §4, kalem kalem). Gerilemeyi `theme.test.ts` görememişti çünkü ezen dosyaya bakmıyordu; o kör nokta da kapatıldı. **Canlı kip bu teslimde yeniden ölçülmedi** — eski 23 rakamı kaynaktan daha eski bir görüntüden gelir, ölçmeden güncel diye yazılmıyor. **Planlanan 9 ekranın 7'si çalışıyor** (`docs/16` §2): Filo, Tekil Pano (içinde 3D termal ikiz sekmesi), Alarm Konsolu, Olay Analizi / Kara Kutu, Trend & Korelasyon, Cihaz Sağlığı, Bölge Haritası. Kapsam dışı bırakılanlar: Ayarlar ekranı ve mobil PWA. Operasyon döngüsü kapalıdır: uyarı → **Neden / Ne doğrulanmalı / Ne yapmalı / Ne kadar acil** → Onayla/Rafa al → denetim izine kaydı. Sesli ikaz, CSV vardiya raporu, 96 ilçe coğrafi harita. |
...
| 9 | **Yenilikçilik & Fizik Tabanlı AI** | `docs/05` §3 · `firmware/` · `libs/panoalgo/panoalgo/detect.py` | Sabit eşik yerine Recursive Least Squares (RLS) ile dinamik ısıl direnç ($K/K_0$) öğrenimi: `S1_loose_conn` fikstüründe `ALM-K-WARN` 226,5. saatte, sabit 70 K eşikli `ALM-THR-TERM-ALM` 399,0. saatte çıkar — **172,5 saat erken** (aynı sayı §2'de de yazılıdır; `data/fixtures/S1_loose_conn.csv`'den doğrulanabilir). **Kenar ile merkez kanıtlanmış biçimde aynı algoritmayı koşar:** saf C çekirdeği (`malloc` yok, sabit bellek) ile Python referansı ortak test vektörü üzerinde her adımda karşılaştırılır (`firmware/tests/test_rls.c`, 240 adım, K + τ + `excited`). Ölçülen en büyük göreli fark: `double` derlemede **1,36e-08**, gerçek MCU hedefi olan `float` derlemede **6,96e-06** (tolerans 1e-4; fark bilinçli bir hız/bellek takasıdır, `firmware/tests/test_rls.c` başlığında gerekçelendirilmiştir). Nokta başına bellek: **384 B** (double) / **196 B** (float) → 25 nokta = 9,4 KB. |
...
# S1: Gevşek Bara Bağlantısı (K/K0 artışı, sabit eşikten 172,5 saat önce erken uyarı)
...
**Sonuç Kanıtı:** Modbus register'ı = REST × 10'un **yarım yukarı yuvarlanmışı**
(`floor(x × 10 + 0,5)` — `backend/app/scada/encoder.py:353`, `docs/03` §Ölçek), IEC 104 değeri = REST
değerinin 0,1 °C'ye yuvarlanmışı. **Python'un `round()`'u burada kullanılmaz:** bankacı
yuvarlaması tam `x,x5` sınırında 1 LSB (0,1 °C) sapar — ör. 55,05 °C → register **551**, `round()` ise 550. Üç taşıma da aynı bellek-içi `PanelImage`'ı, dokümante edilen **0,1 °C register ölçeğinde** yayınlar: `conn_temp` 25 noktanın tamamında **fark 0**. (Ölçek `docs/03` ve `docs/04`'te her nokta için tablolanmıştır; ham REST değeri iki ondalık taşır, taşıma katmanı onu register çözünürlüğüne indirir.)
...
```bash
# Algoritma ve fizik motoru testleri — 500 test, 0 atlanan
cd libs/panoalgo && pytest

# Backend, veritabanı, SCADA, bildirim ve Telegram testleri — 896 geçer / 37 atlanır (933 toplanır)
cd backend && pytest

# Frontend birim + bileşen testleri ve TypeScript tip doğrulaması — 166 test, 0 hata
...
# Arayuz uctan uca: 7 ekran + NotFound gezilir, konsol hatasi sayilir,
# assets/ekran/ goruntuleri uretilir, @axe-core ile WCAG 2.1 AA taranir.
cd frontend && npm run e2e                          # ornek veri kipi (:5173, Vite'i kendi kaldirir)
GRIDUP_E2E_KIP=canli npx playwright test             # canli yigin (:3000 ONCEDEN ayakta olmali)
```
...
TEST_DB_DSN="postgresql://postgres:gridup@127.0.0.1:5432/gridup" pytest   # backend/ içinde → 933 test toplanır
...
python scripts/sir_taramasi.py                   # sır/sızıntı taraması (534 izlenen dosya; sayı depoya dosya eklendikçe değişir)
