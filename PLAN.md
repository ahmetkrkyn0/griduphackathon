...
| T0.5 Üç parçalı compose | ✅ | 15 Eylül: sıfırdan klon + sıfırdan volume ile `up -d --build` → dokuz servisin dokuzu sağlıklı (build 1m22s). `scripts/duman-testi.sh` 24/24 |
...
- [x] **Üç dalı aç ve push et** — T0.6 Adım 3 (`ahmet/backend`, `berke/frontend`, `tuna/veri-ureteci` — üçü de `origin`'de ve `main`'de birleşti)
...
- [x] **Adım 6: `firmware/host/main.c`** — `panobeyni-sim`: sanal seri porttan Modbus master döngüsü (1 s), çekirdek algoritmaları, Modbus slave sunumu (`contracts/modbus-map.yaml`'dan üretilen tablo), MQTT yayını, 7 günlük halka tampon (dosya).
  - **Sapma notu (15 Eylül).** Bu kutucuk 14 Eylül'de işaretlenmişti ama adımın yarısı yapılmamıştı:
    `firmware/host/main.c` yalnızca **hesap çekirdeğini** koşturur (stdin → register tablosu JSON),
    taşıma katmanı yoktu ve `sim/panobeyni_sim.py` hiçbir dalda var olmamıştı. Sonuç: depoda hiçbir
    Modbus **istemcisi** yoktu, cihaz simülatörleri kimse okumadan duruyordu.
    15 Eylül'de `sim/panobeyni_sim.py` yazıldı — MPR-53CS ve TVOC-2'den **gerçek Modbus TCP** ile
    okur, kenar boru hattından geçirir, MQTT'ye yayınlar, halka tamponu vardır (12 test,
    `sim/tests/test_panobeyni_sim.py`). **Hâlâ yapılmayan üç şey** ve gerekçeleri `docs/17` DH2'de:
    (a) sanal seri port yerine Modbus **TCP**, (b) kenarda Modbus **slave** sunumu yok — aynı harita
    merkezde sunuluyor (`backend/app/scada/`, :502), (c) taşıma katmanı C değil Python; çekirdeğin
    aynı olduğu C ↔ Python 1,4e-8 farkla ölçülmüştür.
...
GitHub handle'ları: `@tunadeniz1304` (A), `@ahmetkrkyn0` (B), `@_____` (C) → `CODEOWNERS`'a yazıldı.
C'nin handle'ı commit geçmişinden çıkarılamıyor (commit'leri gmail adresiyle atılmış); Berke kendi handle'ını `CODEOWNERS`'a yazmalı.
...
| 15 Eyl | A + B + C (birleşme sonrası kapatma turu, `Fix/Eksikler`) | **K4** TVOC-2 ID 248 gerçekten sessiz: iddia ölçülünce YANLIŞ çıktı (pymodbus varsayılanı 0x8B kod 11 döndürüyordu), `ignore_missing_slaves=True` ile düzeltildi, 3 ham-soket testi (düzeltme öncesi kırmızı doğrulandı). **K1** senaryo oynatma: `panosim --scenario`; oynatma ile fixture üretimi AYNI yürütücüyü kullanıyor (`scenarios.iter_samples`), 20 fixture ve `docs/12` bayt bayt aynı üretiliyor; `s0`–`s6` gerçek CLI'ya bağlandı. **K3** karar verildi ve yazıldı: fizik hızlandırılmış kalır, `ts` DUVAR SAATİ (`docs/14` §7). **K5** merkez dedektör bağlandı; bağlanır bağlanmaz B'nin elle yazdığı test yükünde kenarın bildirmediği iki gerçek eşik ihlali buldu. **Y2** `sim/panobeyni_sim.py` yazıldı — gerçek Modbus TCP master → kenar → MQTT, halka tampon, 12 test; `--trip-after` ile üretilen ark tripi artık `ALM-ARC-TRIP` olarak yayına çıkıyor. **Y8** yük testi `--generator physics` (panoalgo kütüphane olarak import). **Y4** T4.4 temiz makine testi koşuldu: sıfırdan klon + `down -v`, 24/24. **K2+Y3** README, sunum taslağı ve `docs/17` birleşme sonrası gerçeğe göre düzeltildi. Ayrıca sözleşme okumaları önbelleğe alındı: panoalgo test takımı **452 s → 53 s**, üreteç kurulumu 173 ms → 0,31 ms/pano. Test: panoalgo 331/331, backend 583 + 22 atlandı, sim 33/33, frontend 71/71 | Y6 (KiCad şema PDF ve STL) ekip kararı bekliyor — OpenSCAD/KiCad bu makinede kurulu değil; Y5 iki sözleşme önerisi karar-hazır ama **onaylar işaretlenmedi** (üç imza gerekir); O4 gerçek telefona WhatsApp Meta token'ına bağlı | Birleşmenin kendisi iki gizli hata üretmişti, ikisi de düzeltildi: `gsm-modem` backend Dockerfile'ını paylaşıyordu ve context değişince TÜM yığını düşürüyordu (yalnızca temiz kurulumda görünüyordu); kenar tetiklemeli `ALM-ARC-TRIP` raporlanmayan taramaya denk gelince kayboluyordu (olay mandalı eklendi). `KALAN-EKSIKLER.md`'deki "Faz 0 `hello_publisher` artığı GDZ-00001" tespiti YANLIŞ: o satır `initdb/001_schema.sql` içindeki bilinçli başlangıç verisidir (0 telemetri satırı) |
