...
- [ ] **Adım 1: Alarm konsolu** — öncelik/zaman/konum sıralı liste; her kartta **"Neden?"** (katkı yapan sinyaller + eşikleri, `reason` alanından), **"Ne yapmalı?"** (`advice`), **"Ne kadar acil?"** (`ttl_h`); onay/raf/yorum butonları; eskalasyon sayacı. P1'de görsel + sesli.
- [ ] **Adım 2: Dijital ikiz** — `assets/ek2-14-pano.svg` üzerinde sensör noktaları; `t_conn[].dt_c`/`k_ratio`'ya göre renklendirme; tıklayınca o noktanın trendi. **Gizlilik:** komitenin PDF'i kullanılmaz, ölçülere sadık kendi SVG'miz çizilir (rapor §12.1).
- [ ] **Adım 3: KiCad şeması v1** — gerçek parça numaralarıyla: MCU modülü (ESP32-S3-WROOM-1 veya STM32G4 + modül), 2× izoleli RS485 (ADM2587E sınıfı), izoleli AC/DC SMPS girişi, süperkapasitör yedek, 2× röle, 4× optokuplörlü giriş, 3× AT girişi (burden + ADC), hücresel modem konnektörü, secure element, DIN kutu. **Hiçbir geliştirme kartı şemada yok** (GK5).
- [ ] **Adım 4: `io-tablosu.md`** — her pin: sinyal, yön, gerilim seviyesi, izolasyon sınırı, bağlandığı blok.
- [ ] **Adım 5: `docs/16`'yı yaz** + her ekranın görüntüsünü `assets/ekran/` altına al.
- [ ] **Adım 6: Commit** — `feat(frontend): alarm konsolu ve EK-II/14 dijital ikiz` + `feat(hw): Pano Beyni sema v1 ve I/O tablosu`
...
- [ ] **Adım 9: Commit** — `feat(backend): Modbus TCP ag gecidi + uretilmis harita dokumani` + `test(loadtest): 1000 pano yuk testi ve olcek olcumleri`
...
- [ ] **Adım 1: Trend/korelasyon ekranı** — **I²–ΔT dağılım grafiği** (sağlıklı vs gevşek bağlantı iki farklı eğim — bu grafik sunumun en güçlü görseli), K trendi, çiy noktası marjı zaman serisi.
- [ ] **Adım 2: Olay analizi (kara kutu)** — ark tripinde dedektör konumu + öncesindeki 72 saatin sinyalleri tek zaman çizelgesinde.
- [ ] **Adım 3: Cihaz sağlığı + bölge haritası** — düğüm başına RSSI/yedek enerji/son görülme/firmware; haritada trafo merkezleri risk rengiyle (offline harita karosu, internet yok).
- [ ] **Adım 4: Yerleşim çizimi** — `ek2-14-yerlesim.svg`: kontrolcü üst bölmede (şartname 2.2.8.1.iv atıflı), termal dizi saydam kapağın **iç** tarafında, nokta düğümleri bara/pabuç eklerinde, ortam düğümleri alt/üst, reed kapıda; her öğenin yanında ölçü ve baradan uzaklık.
- [ ] **Adım 5: Mekanik + BOM** — DIN kutu 3D (OpenSCAD → STL), V-0 malzeme notu; `bom.csv`: parça, üretici kodu, adet, birim fiyat (adet 1) ve (adet 1.000), tedarik notu.
- [ ] **Adım 6: Dokümanları yaz** — `07` (FMEA donanım/saha satırları), `08` (kurulum: 5 güvenlik kuralı, ≤45 dk, 2 kişi, AT sekonderi açık bırakılmaz uyarısı), `10` (SKU'lar + BOM + parametrik ROI tablosu), `11` (standart → nerede karşılandı), `13` (blok diyagram, güç bütçesi, manyetik alan ve clearance/creepage hesapları), `01` (jüri için 3 sayfalık problem özeti).
- [ ] **Adım 7: Commit** — `feat(frontend): trend, kara kutu, cihaz sagligi ve bolge haritasi` + `docs(hw): yerlesim, BOM, FMEA, kurulum ve standart dokumanlari`
...
- [ ] **T4.3 (B): `docs/02-mimari.md`** — mermaid bileşen + veri akışı + sıralama diyagramı (sensör → kenar → broker → ingest → risk → alarm → SMS, gecikme damgalarıyla).
...
- [ ] **T4.5 (C): `README.md`'yi bitir** — amaç, mimari görseli, tek komutla kurulum, demo senaryoları, **"beklenen 7 teknik çıktı → dosya/klasör" eşleme tablosu** (rapor §11 teslim listesi), ekip, gizlilik notu.
- [ ] **T4.6 (C): Ekran görüntüleri** — her ekranın 2 hali (normal/alarm) `assets/ekran/` altına; `docs/16`'ya gömülü.
- [ ] **T4.7 (hep): Sır taraması** — `git log -p | grep -iE "token|password|\+90|api[_-]?key"` temiz; `.env` commit edilmemiş.
...
- [ ] **T5.1 (C): Demo betikleri deterministik** — `demo/senaryo/s0.sh … s8.sh`, her biri seed'li ve aynı sonucu üretiyor. Canlı demoda sürpriz olmaz.
- [ ] **T5.2 (C): Sunum destesi (≈12 dk iskelet, rapor §8.2)** — problem ve saha hikâyesi (30 s) → dökümanlardan çıkardığımız 5 kritik içgörü (1 dk) → mimari (1,5 dk) → demo (7 dk) → ölçek + maliyet/ROI + kurulum (1,5 dk) → PoC teklifi (30 s). **Donanımsızlığı slayt 3'te açıkça ve güçlü biçimde konumlandır** (Bölüm C).
