...
| 11 | Kimlik doğrulama, rol tabanlı erişim, kurumsal SSO | **Yok**; onaylayan adı istemciden gelir | **Doğrulanmadı** | **Bilinçli kapsam sınırı**, üç ayrı yerde yazılı (`docs/15-guvenlik-kvkk.md` §5). Ürünleşmenin ön koşulu (`GELISTIRME-BACKLOGU.md` F-19) |
...
| **CIGRE TB 858** — varlık sağlık indeksleri | Varlık **sağlık indeksi** kurulması ve indeksin **monotonluk** ilkesine uyması: daha fazla ya da daha ağır kanıt indeksi düşürmemeli | Risk skoru 0–100: `libs/panoalgo/panoalgo/fusion.py` (`RiskResult.score`, `contributions`). Monotonluk **özellik testiyle** sınanıyor: `libs/panoalgo/tests/test_fusion.py` — sözleşmedeki kanıt kodlarının **0–3 elemanlı tüm alt kümeleri** taban alınıp her tabana kalan her kod tek tek ekleniyor, üç ayrı özellik kümesiyle | Monotonluk testi geçiyor; `libs/panoalgo` **401/401** test yeşil (16 Eylül 2026, bu dalda ölçüldü). Risk skorunun bildirim kararını **yönetmediği** kodda yazılı: SMS/arama kararı alarm önceliğinden (P1/P2/P3) çıkar, skordan değil | **Risk matrisimiz fiilen tek boyutludur.** `frontend/src/components/RiskMatrisi.tsx` başlığındaki yorum bunu **kendisi itiraf ediyor**: x = sınıra kalan süre, **y = risk skorunun kendisi**; gerçek bir **etki/kritiklik ekseni yok**. Nedeni de orada yazılı — sözleşmede filodaki tüm panolar aynı `pano_type` (1600 kVA) olduğu için kVA sabit bir eksen olurdu ve **sabit bir alanı "etki" gibi göstermek yanıltıcı olurdu**. Ayrıca sağlık indeksi **yaş, bakım geçmişi ve abone etkisi içermiyor**: `panels` tablosunda kritiklik alanı yok (`GELISTIRME-BACKLOGU.md` §1, F-21) |
...
5. **Ticari hazırlığı kanıtlamaz.** Kimlik doğrulama, rol tabanlı erişim, **periyodik filo raporu ve
   veri dışa aktarma** (tek olay için yazdırma var, fazlası yok) ve varlık kütüğü bu teslimde
   yoktur; §a.3'te boşluk olarak işaretlidir.
