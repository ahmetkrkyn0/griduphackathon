...
| Operasyon arayüzü | Kontrol odası | React arayüzü (9 ekranın 7'si) | **Gerçek** | `frontend/` **166 test** (19 dosya; 4'ü bileşen/DOM testi), `docs/16`; 7 ekran **canlı API'de 0 konsol hatası — ölçümü artık `frontend/e2e/smoke.spec.ts` yeniden üretiyor** | C · ✅ |
...
| **S1'de L1 katmanı, sabit 70 K eşiğinden en az 48 saat önce uyarıyor** | ✅ **209 saat (8,7 gün)** — kriterin dört katından fazla. İlk L1 kodu 13 Tem 22:15, 70 K ihlali 22 Tem 15:15 — tetikleyen kod `ALM-TTL-14D` (prognoz); yalnız K/K₀ eşiğiyle öne alma **172,5 saattir** (`docs/12` §2) |
| Duyarlılık (recall) — eşleşen model | ✅ **1,00** — beklenen alarmı olan 8 senaryonun hepsinde; S0 ve S6'da beklenen alarm yoktur (S6'nınkini merkez üretir) |
| Duyarlılık (recall) — **uyumsuz model** | ⚠️ **0,88** — dedektörün varsaymadığı fizik eklendiğinde (S10–S13). Bu bir kusur raporu değil, **yöntemin ölçülmüş sınırıdır**: `docs/12` §1.1 |
...
|---|---|---|
| `libs/panoalgo` (A) | `cd libs/panoalgo && pytest` | **498 / 498** (19 Eylül'de yeniden koşuldu) |
...
| Backend (B) | `cd backend && pytest` | **896 geçti / 37 atlandı** (20 Eylül, `TEST_DB_DSN` **verilmeden**; toplanan 933). **DSN'li sayı bu oturumda yeniden ölçülmedi** — tohumlayıcı münhasır veritabanı ister, canlı yığın ayaktaydı (bkz. `README.md` Testler bölümü) |
| Frontend (C) | `npm ci && npm test && npm run build` | **166 / 166** (19 dosya, vitest 2.1.9), `tsc --noEmit` **0 hata** (20 Eylül) |

> Sayı zinciri: panoalgo 331 → 401 → **498**, backend 583 → 661 → **880 (+37 atlandı)**,
> frontend 71 → 85 → **154**. Son sütun 19 Eylül ölçümüdür.
>
> **19 Eylül düzeltmesi — bu not kendi kendini yanlışladı.** Önceki hâli *doğru kaynak bu
> tablodur* diyordu; oysa tablo 85'te kalmışken `README.md` zaten 136'yı yazıyordu. Yani
> tablo, kendini tek doğru kaynak ilan ederken **depodaki en bayat sayıyı** taşıyordu.
> Ders kayda geçiyor: *doğru kaynak burasıdır* bir garanti değil, **bakım borcudur**.
> Frontend sayısı için tek kaynak artık `npm test` çıktısının kendisidir.
...
- 7 ekran gerçek API ile Playwright'ta gezildi: **0 konsol hatası / uyarısı**.
  **19 Eylül — bu satır artık yeniden üretilebilir ve ölçüm bir kusur buldu.** Tezgâh depoya
  girdi (`frontend/playwright.config.ts`, `frontend/e2e/smoke.spec.ts`); koşumu:
  `cd frontend && GRIDUP_E2E_KIP=canli npx playwright test`. İlk koşuda **kara kutu ekranı
  (`/olay/:id`) tamamen boş çıktı** — React #310, "Rendered more hooks than during the previous
  render"; `OlayAnalizi.tsx`'te bir `useMemo` erken dönüşlerin altındaydı. Yani bu satırın
  yazıldığı sırada **7 ekranın 7'si değil 6'sı** hatasızdı. Kusur düzeltildi, regresyonu
  `frontend/src/pages/OlayAnalizi.test.tsx` ile kilitlendi ve ölçüm yenilendi:
  **canlı kipte 10 rotada 0 hata / 0 uyarı** (örnek veri kipinde de aynı).
  **Bu sayının kapsamı dar yazılır:** tek tarayıcı (Chromium 153), tek genişlik (1425 px),
  yalnızca **sayfa açılışı** — tıklama sonrası ve 3B ikiz sekmesi ölçülmedi.
...
| 9 | Maliyet ne kadar, geri ödemesi ne? *(rapor §13 #11)* | `hardware/pano-beyni/bom.csv` satır kalemlerinin toplamı: **adet 1'de 70,73 USD**, **adet 1.000'de 47,68 USD** (**19 Eylül düzeltmesi:** bu satır ~56/~37 USD diyordu; fark tam olarak hücresel modem kalemidir). Pano başına **toplam** (kontrolcü + N sensör düğümü) adet 1.000'de **103,48 – 396,43 USD**'dir ve N sözleşmeyle sınırlıdır (`docs/10` §7). SIM/veri aboneliği, kurulum işçiliği ve tip test **hiçbir toplamda yoktur ve ölçülmedi**. Mevcut MPR-53CS ve TVOC-2 Modbus'tan sensör olarak okunduğu için BOM'a girmeyen kalemler haritadan sayıldı (4 akım trafosu, 3 gerilim girişi, 2 ark dedektörü, 1 ark merkez ünitesi) — ama **birim fiyatları depoda yok, bu yüzden net fark "veri yok"**. Buna karşılık ödenen kalem ölçülüdür: izole RS485 arayüzü 4,90 USD (adet 1) / 3,70 USD (adet 1.000). ROI tablosu bir **hesaplayıcı taslağıdır** ve varsayımlarını kendisi işaretler (P(arıza) %3, arıza başı 8.000 USD, tespit oranı ölçülen 1,00 yerine **bilinçli iskontolu %70**). **19 Eylül'de ölçüldü:** `--duyarlilik` bu üç varsayımın kaldıracının **birebir eşit** olduğunu gösterdi (1,33×, çarpımsal model), yani biri düzeltilmeden hesap düzelmez; geri ödeme tek sayı olarak değil **aralık** olarak yayımlanıyor (`docs/10` §7.3). Maruziyet tarafında ise tarifenin kaldıracının **üstten 1,00× ile sınırlı**, eşiğe olan mesafeninkinin **sınırsız** olduğu ölçüldü (§7.4) — bu bölümün ilk hâlindeki *"sıralama değişmez"* iddiası yanlıştı ve düzeltme gerekçesiyle yazıldı. Ölçülebilir olan taraf ayrı tutuldu: `scripts/tazminat_maruziyeti.py` parametresiz çalıştırıldığında hesap yapmaz, yönetmelik eşiğini ve tarifeyi **uydurmaz**. "Şu kadar arıza önledik" cümlesi hiçbir yerde geçmez. Kanıt: `docs/10`. |
| 10 | Operatör bu alarmla ne yapacak? | Her alarm kartı üç soruyu cevaplar — **Neden / Ne yapmalı / Ne kadar acil** — artı karşı-olgusal dördüncü blok: **"Ne doğrulanmalı"** (teşhisi kesinleştirecek eksik kanıt). Yönetim ISA-18.2 yaşam döngüsüdür: onay, **süreli ve gerekçeli raf** (P1 rafa alınamaz — API 403), bakım modu (P1 asla bastırılmaz), gruplama, histerezis, eskalasyon (P1'de 5. dakikada arama, 15. dakikada üst amir). Hedef öncelik dağılımı %5 / %15 / %80 (EEMUA 191). Arayüz ISA-101 / yüksek performanslı HMI dilindedir: nötr/sakin zemin (14 Eylül revizyonunda RAL 7035 grisi yerine açık nötr — **19 Eylül ölçümü: `theme.css:3` `--bg: #f5f6f8`**, `theme.test.ts` ile kilitli), renk yalnızca anormal durumda, öncelik **renk + şekil + karakter** ile (renk körlüğü); açılış ekranı bir gösterge panosu değil **"sınıra kalan süre" zaman eksenidir**. Kanıt: `docs/06`, `docs/16`, `frontend/` **154/154** test (17 dosya; bunların 5'i gerçek DOM'da çizim yapan bileşen testidir — alarm kartının dört başlığı, belirteçsiz "Onayla" → 401, boş filo yanıtı, eksik `reason` alanı, kara kutu geçişi), 7 ekran canlı API'de **0 konsol hatası** — ve bu sayı artık `frontend/e2e/smoke.spec.ts` ile **yeniden üretilebilir**. **Erişilebilirlik dürüstçe:** aynı spec `@axe-core/playwright` ile tarıyor ve **örnek veri kipinde 0 ihlal** ölçüyor (20 Eylül; kontrast dâhil hiçbir WCAG 2.1 AA kuralı ihlal edilmiyor) ve sayı testte kilitlidir. Bu sayı 19 Eylül'de 5, 20 Eylül sabahı **83**'tü; artış `main` birleşmesiyle gelen sanayi kabuğunun `--dim` token'ını ezmesinden doğan bir **gerilemeydi**, `docs/16` §4'te kalem kalem sayılı ve dört kök rengin dördü de ölçülerek düzeltildi. |
...
| 14 | 10.000 panoya nasıl ölçeklenir? *(rapor §13 #12)* | **Ölçtük:** tek backend süreci 5.000 panoya kadar görünme p95 < 1 s; 10.000'de veri kaybetmeden doyuyor ve darboğaz ölçüldü — mesaj başına 615 µs'in **501 µs'i şema doğrulaması**. Kaldıraçlar: uyarlanabilir raporlama ve paylaşımlı abonelikle çoklu ingest. **18 Eylül'de uyarlanabilir raporlama uygulandı ve ölçüldü** (F-36): "÷6" bir tahmindi, ölçülen **÷1,70** (docs/09 §6.1) — kaldıraç gerçek ama tahmin edilenden küçük; alarm anı değişmedi. Depolama: sıkıştırma **46–48×** ölçüldü ve şemaya eklendi; 100 pano 10 s'de ~105 GB/yıl, ölçülen orana göre ~62 GB/yıl (docs/09). |
| 15 | Siber güvenlik? *(rapor §13 #13)* | Kodda olanlar: sözleşme dışı veri karantinası, pano kimliği denetimi, Modbus IP listesi + salt okunur varsayılan + koruma cihazına yazma yasağı + kaba kuvvet kilidi, denetim izi (docs/15). **18 Eylül'de ikisi koda geçti:** REST yazma uçlarında operatör belirteci ve rol (F-19, `backend/app/auth.py`), MQTT'de **mTLS + cihaz başına topic ACL** (F-27, `deploy/mosquitto.acl`) — ikincisi canlı brokerda ölçüldü: bir panonun sertifikasıyla başka panonun topic'ine yayın **PUBACK reason code 135 ile reddedildi**, abone mesajı **almadı**, broker logunda `Denied PUBLISH` (docs/15 §5.1). Tasarımda kalanlar: sertifikanın **güvenli elemandan** gelmesi, imzalı OTA (A/C). **Kalan sınırlar:** mTLS **ayrı bir profildedir ve varsayılan kapalıdır** (`--profile mtls`); varsayılan demo yolunda broker hâlâ anonim 1883'tür; REST/WS, Modbus TCP ve IEC 104 her iki kipte de düz metindir; okuma uçları belirteç istemez; sertifika iptali (CRL/OCSP) yoktur. Üretim farkları: docs/15 §5. |
...
| 7 | ~~Cihaz Sağlığı ekranı pano başına istek atıyor~~ → **18 Eylül'de kapandı** | Öneri üç onayı aldı; `GET /api/v1/fleet/health` eklendi (`openapi.yaml` **v1.1.0**, yalnızca ekleme) ve ekran artık **tek istek** atıyor. Uygulama sırasında ölçülerek iki sapma bulundu ve düzeltildi: `fw` yükün kökünde duruyor (`health` bloğunda değil) ve `maint_mode`/`baseline_day` taslak şemada eksikti. İki ucun aynı alan için aynı değeri döndürmesi testle kilitli (`test_fleet_health_matches_panel_detail`). **Kalan dürüstlük sınırı:** 100+ panodaki kazanç **ölçülmedi** — demo filosu 3–20 pano ve bu ölçekte fark zaten görünmüyordu; değişen şey istek sayısıdır (N → 1), ölçülmüş bir gecikme iyileştirmesi değil | `docs/16` §3, `contracts/changes/2026-09-14-fleet-health-bulk.md` |
...
| 10 | Varsayılan demo yolu büyük ölçüde kimlik doğrulamasız (broker anonim 1883, Modbus/IEC 104 düz TCP, **taşımada TLS yok**) — **REST yazma uçları ve MQTT tarafı 18 Eylül'de daraltıldı** | Varsayılan yol tek makinede, internetsiz ve **sertifika gerektirmeden** çalışsın diye sadeleştirildi (GK4). 18 Eylül'de iki kez daraltıldı: **F-19** ile onay/raf operatör belirteci ister ve onaylayanın kimliği doğrulanmış başlıktan gelir; **F-27** ile MQTT için ayrı bir profil (`--profile mtls`) eklendi — 8883'te mTLS, cihaz başına topic ACL, canlı ölçümle (`docs/15` §5.1). **Boşluk daraldı, kapanmadı — kalan sınırlar:** mTLS **varsayılan kapalıdır**, `docker compose up -d` broker'ı hâlâ anonim 1883'te başlatır; **Modbus TCP (502) ve IEC 104 (2404) her iki kipte de düz TCP'dir** ve bu bilinçlidir (gerekçe ve ~45 satırlık maliyet ölçümü `docs/15` §5.1); REST/WS'te HTTPS yok, okuma uçları ve WS akışı belirteç istemez; Grafana anonim izleyici; belirteçler paylaşılan sırdır (kurumsal SSO değil) ve `GRIDUP_OPERATORS` boşsa kimlik doğrulama tamamen kapanır; sertifikalar **elle** üretilir, **iptal (CRL/OCSP) yoktur**. İki durum da `GET /health`'te görünür (`auth.enabled`, `mqtt_tls`). Sadeleştirmelerin **hiçbiri koddaki kontrolleri kapatmaz** | `docs/15` §5 ve §5.1, `contracts/changes/2026-09-18-kimlik-dogrulama.md` |
...
| 14 | ~~Üç alarm kodunun eşiği sözleşmede yok~~ → **18 Eylül'de kapandı (ikisi için)** | Öneri üç onayı aldı ve `alarm-codes.yaml` **v2** ile uygulandı: `ALM-DQ-BELOW-AMBIENT` ve `ALM-NEUTRAL-THD` artık sözleşmedeki eşiklere atıf yapıyor (ikincisi **iki** eşiğe birden — alan artık liste kabul ediyor ve doğrulayıcı her üyeyi ayrı denetliyor). Davranış değişmedi: değerler kodda duran türetilmiş varsayılanlarla birebir aynı, değişen tek şey sayıların **nerede yaşadığı**. `ALM-PD-TREND` **bilinçli olarak eşiksiz kaldı** — AG panoda `pd: null` geldiği için değerlendirilecek veri yok; eşik yerine `scope:` notu düşüldü. **Kalan dürüstlük sınırı:** yeni üç sayıdan üçü (`1.0`, `0.30`, `15.0`) **türetilmiştir, ölçülmemiştir** ve sözleşmedeki yorumlarında böyle yazıyor; yalnızca `excitation_min_cv_i2 = 0.02` ölçülmüş bir taramadan gelir | `docs/05` §10, `contracts/changes/2026-09-14-eksik-esikler.md` |
...
| 17 | ~~Sensör düğümleri kavramsal seviyede~~ → **19 Eylül'de kapandı (kısmen)** | Düğüm ve PD kartı dizinleri artık deponun kendi olgunluk üçlüsünü taşıyor (blok diyagramı + I/O tablosu + BOM; düğümde ayrıca enerji/termal hesap). Pano başına toplam maliyet artık **ölçülü**: 103,48 – 396,43 USD (adet 1.000, düğüm sayısına göre). **Kapanmayan kısım:** hiçbir BOM satırı veri sayfasına karşı doğrulanmadı, kart üretilmedi, kablosuz menzil metal kabinde ölçülmedi. Kurulum işçiliği ve SIM/veri aboneliği hâlâ **hiçbir toplamda yok** | `docs/10` §2 ve §7, `docs/19` §3, `hardware/sensor-dugumu/` |
...
| 19 | Yük testinde 1.000–10.000 ayrı TLS bağlantısı ve alarm seli ölçeği sınanmadı; koşular 2–5 dakika | 20–50 MQTT bağlantısı panoları paylaştı. **18 Eylül'den sonra TLS yolu VAR (F-27) ama yük testi hâlâ düz 1883'te koştu**: `loadtest/fleet.py` sertifika almıyor, dolayısıyla TLS el sıkışma CPU maliyeti ve toplu yeniden bağlanma fırtınası **hâlâ ölçülmedi** — ayrıca mTLS kipinde topoloji de değişir (N pano = N bağlantı, bugünkü paylaşılan bağlantı değil). Yüzlerce eşzamanlı P1 ayrıca sınanmalıdır. 24 saatlik etkiler (sıkıştırma işinin kendisi, autovacuum, parça oluşturma) ölçümün dışındadır | `docs/09` §8 |
| 20 | **Denetim izi hash zincirinin kapsamadığı iki şey** (F-20, 18 Eylül) | Zincir eklendi ve ölçüldü: değiştirilen satır 2. halkada, silinen satır ardındaki halkada yakalanıyor (`scripts/verify_journal.py`, çıkış kodu 1). Kapsamadığı iki şey **bilinçlidir**: (a) **kuyruk kesme görülemez** — zincirin *son* satırları silinirse kalan zincir kendi içinde tutarlıdır ve doğrulayıcı "sağlam" der; kapatmak için zincir başının dışarıya (WORM depo / zaman damgası otoritesi) yayınlanması gerekir ve **yapılmadı**. Bu sınır testle kilitli: `test_tail_truncation_is_NOT_detected`. (b) Özet **anahtarsızdır** (HMAC değil): veritabanına *yazma* yetkisi olan biri satırı değiştirip zinciri baştan hesaplayabilir. Hedeflenen tehdit "sessizce bir satır silen yetkili kullanıcı", "zinciri yeniden kuran saldırgan" değildir | `docs/15` §3.3, `backend/app/journal_chain.py` |
| 21 | Ekran görüntüleri örnek veri modunda alındı | `npm run dev:mock` ile alındı ve `docs/16` §5'te böyle işaretlendi; gerçek yığından yenilenmesi aşağıdaki "hâlâ açık" listesindedir | `docs/16` §5 |
| 22 | **Varlık künyesi açıldı ama BOŞ** (F-21, 18 Eylül) | `panels` tablosuna CBS tekil kodu, fider, il/ilçe, abone sayısı, kritiklik, trafo gücü ve bakım tarihleri eklendi (göç `deploy/initdb/008_varlik_kutugu.sql`, sözleşme `openapi` **v1.3.0**) ve doğrulanmış bir içe aktarım ucu yazıldı (`POST /fleet/assets`, `muhendis` rolü, 14 test). **Bu teslimde içe aktarılmış gerçek veri yoktur:** gerçek bir CBS dışa aktarımına erişimimiz yok (GK3), demo filosunun künyesi **boştur** ve `uretici`/`seri_no` **hiçbir panoda doldurulmadı** — uydurulmadı, boş bırakıldı. Eksiklik gizlenmiyor: `GET /fleet/assets` kapsama oranını **sayıyla** verir, künyesiz pano `asset: null` döner ve risk matrisi künyesiz panoları y=0'a **koymaz**, ayrı bir şeritte gösterir. Risk matrisi bu yüzden **künye girildiği ölçüde** iki boyutludur; künye yoksa eski tek boyutlu davranış **aynen** korunur (testle kilitli: `frontend/src/lib/etki.test.ts`). `kritiklik` sözlüğü bizim tanımımız **değildir** — erişilebilir bir standart sınıflandırma bulunamadı ve GK10 gereği erişemediğimiz bir metnin madde numarası yazılmadı | `docs/16` §3, `docs/18` (CIGRE TB 858 satırı), `contracts/changes/2026-09-18-varlik-kutugu.md` |
| 23 | **Kesinti bağıntısı toplayıcıdır, susturucu değil** (F-22, 18 Eylül) | Aynı fiderdeki panoların eş zamanlı susması artık N ayrı arıza değil **tek bir kesinti olayıdır** (`backend/app/outage.py`, göç `009_kesinti_olayi.sql`, sözleşme `alarm-codes` **v3** + `openapi` **v1.4.0**). **Bu dilimde alarm BASTIRILMADI:** alt alarmlar (`ALM-LASTGASP` P2, `ALM-COMMS-LOST` SYS) aynen üretilir ve bildirim davranışı **değişmez**; olay onları yalnızca `outage_id` ile bağlar. Bastırma bir ISA-18.2 kararıdır (kimin, hangi koşulda bastırdığı denetlenebilir olmalı) ve en yüksek öncelikli alarmın asla bastırılmadığı bir durum makinesi ister — F-25'in konusu. **Eşiklerin ikisi de TÜRETİLMİŞTİR, ölçülmemiştir** (`outage_min_panels: 3`, `outage_window_min: 2`) ve sözleşmedeki yorumlarında böyle yazar: bu depoda gerçek fider topolojisi ve gerçek kesinti kaydı yok (GK3). **Tek panolu durumda eski davranış birebir korunur** (eşik 3) ve bu adı konmuş bir testle kilitli: `test_tek_pano_susunca_eski_davranis_aynen_korunur`. **Beyan edilmiş risk:** bağıntı gerçek bir şebeke topolojisinden değil **içe aktarılmış bir etiketten** (F-21 `fider_id`) türer — yanlış fider eşlemesi N panoyu yanlış tek olaya toplar; fideri bilinmeyen pano bu yüzden hiç gruplanmaz. Demo filosunun künyesi boş olduğu için bağıntı **demo veritabanında hiç tetiklenmez**. `ended_at` enerjinin geri geldiği an **değildir**, haberleşmenin döndüğü andır ve histerezislidir | `contracts/changes/2026-09-18-kesinti-bagintisi.md`, `backend/app/outage.py` |
| 24 | **EPDK Madde 8 kaydı TASLAKTIR; on bir alanın altısı elle doldurulur** (F-23, 18 Eylül) | Kesinti olayı artık EPDK Kalite Yönetmeliği (RG 29/12/2020, 31349 mük.) **Madde 8/2** alanlarıyla doldurulmuş bir taslağa çevriliyor (`backend/app/epdk.py`, `openapi` **v1.5.0**). **TASLAK olması yanıtın yapısıdır**, iliştirilmiş bir uyarı değil: her alan `olculen` / `oneri` / `elle_doldurulacak` durumu taşır ve alan yanıttan **hiçbir zaman düşürülmez** — çıkarmak "bu alanı ölçmüyoruz" bilgisini de kaybettirirdi. **On madde, on bir alan:** mevzuat "başlama/sona erme"yi tek kalem sayar, biz ikiye ayırdık çünkü **başlama ölçülüyor, sona erme ölçülmüyor**. **Üçü ölçülüyor** (yer — F-21 CBS tekil kodundan; etkilenen kullanıcı — F-22'den; başlama — bir **yaklaşımdır**, panoların sustuğu andır), **ikisi öneri** (neden, sınıf — hava/ağaç/hayvan gibi bir sebep **tahmin edilmez**), **altısı elle doldurulacak**. En önemli sınır: **restorasyon anı ölçülmüyor** — elimizdeki tek şey haberleşmenin dönüşüdür ve o da 5 dk histerezislidir, enerji dönüşü **değildir**; sona erme olmayınca süre, toplam etkilenme süresi ve dağıtılmayan enerji de üretilemez. **Tazminat hesabına girilmedi** (GK10): formül parametrelidir, `SBSÜRE` literal bir tutar değildir ve `scripts/tazminat_maruziyeti.py` zaten parametresiz çalıştırıldığında hesap yapmaz. **Yeni zaman çizelgesi üretilmedi:** `GET /events/{event_id}/blackbox` zaten vardı ve F-02 pencereyi 336 saate çıkarmıştı — taslak o çizelgeye **bağlanır** | `contracts/changes/2026-09-18-epdk-madde8-taslagi.md`, `backend/app/epdk.py` |
...
sağlanmadı, `ttl_h` null kaldı — uydurmak yerine susuyor); `S8_sensor_fault`'ta ise sınır **hiç aşılmadığı hâlde 183 tahmin**
üretildi ve **86'sı** `ALM-TTL-14D` alarmına döndü. Bu bir **prognoz yanlış-alarmıdır** ve `docs/12` §3'teki yanlış alarm
sayacı onu görmez, çünkü etiket penceresinin içinde çıkıyor. Kaynağı da ölçüldü: 183 tahminin tamamı S8'in **sürüklenen**
...
olarak değil; manşetteki **209 saatlik öne almayı tetikleyen kod `ALM-TTL-14D`'dir, yani prognozdur; K/K₀ eşiğine dayanan öne alma 172,5 saattir** ve ikisi
