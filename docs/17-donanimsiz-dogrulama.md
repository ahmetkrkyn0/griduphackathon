...
| 10 | Demo yığını büyük ölçüde kimlik doğrulamasız (broker anonim 1883, Modbus/IEC 104 düz TCP, **TLS hiç yok**) — **REST yazma uçları 18 Eylül'de kapatıldı** | Demo tek makinede, internetsiz ve sertifika altyapısı kurmadan çalışsın diye sadeleştirildi (GK4). 18 Eylül'de F-19 ile REST tarafı daraltıldı: onay/raf artık operatör belirteci ister ve onaylayanın kimliği istemciden değil doğrulanmış başlıktan gelir. **Kalan sınırlar aynen duruyor:** okuma uçları ve WS akışı belirteç istemez, taşıma şifresiz, Grafana anonim izleyici, belirteçler paylaşılan sırdır (kurumsal SSO değil) ve `GRIDUP_OPERATORS` boşsa kimlik doğrulama tamamen kapanır (`GET /health` bunu söyler). Sadeleştirmelerin **hiçbiri koddaki kontrolleri kapatmaz** | `docs/15` §5, `contracts/changes/2026-09-18-kimlik-dogrulama.md` |
