...
| 4 | S8 (sensör arızası) sınır aşılmadan alarm üretmiyor | `docs/12` §1 (recall 1.00, tek beklenen alarm = arıza tespiti, sınır ihlali değil) | **99 sonlu TTL tahmini üretildi**, sınır HİÇ aşılmadı (`docs/12` §4.3, "PROGNOZ YANLIŞ-ALARMI"); 89'u `ALM-TTL-14D` alarmına döndü ve §3'teki yanlış-alarm sayacı bunu GÖRMÜYOR (etiket penceresi içinde çıkıyor). **Kök neden:** kalite katmanı (`quality.py`) yavaş/monoton sürüklenmeyi yakalamıyor + TTL hattı (`edge.py:109-110`, `detect.py:292-305`) kalite bitlerinden habersiz — RLS regresyonu yavaş sensör sürüklenmesi ile gerçek ısınmayı ayıramıyor. Zaten `docs/05` §10 ve `GELISTIRME-BACKLOGU.md:87-92` (F-04), `:293`'te belgeli; **README/PLAN.md'de yok**. |
...
3. **S8 sahte-TTL alarmı** (99 tahmin, sınır hiç aşılmadan) — kod düzeyinde düzeltme yapılmadı, yalnız belgelendi (`docs/05` §10). Kalite bitlerinin TTL hattına bağlanması gerekiyor (`edge.py`/`detect.py`).
