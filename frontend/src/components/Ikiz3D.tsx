...
import { BAR_Y, DSYA_COUNT, FIRST_SPARE_DSYA, GIRIS_N_X, N_BAR_Y, PANEL_MM, dsyaX, girisX, pointPos3d } from "../lib/panelGeometry";
...
const HOME_OFFSET = new THREE.Vector3(1650, 700, 2600);
const HOME_TARGET = new THREE.Vector3(0, 720, 0);
const ALL_POINTS = [
  ...Array.from({ length: DSYA_COUNT }, (_, i) => PHASES.map((p) => `DSYA${i + 1}_${p}`)).flat(),
  "GIRIS_L1",
  "GIRIS_L2",
  "GIRIS_L3",
  "GIRIS_N",
...
const isAbnormal = (s: PointState) => s === "warn" || s === "alarm" || s === "critical";

function buildScene(stage: HTMLDivElement, tip: HTMLDivElement, onPick: (pt: string) => void) {
  ...
  renderer.domElement.setAttribute("aria-label", "Panonun 3D dijital ikizi. Ölçüm noktalarının durumu aşağıdaki tabloda da listelenir.");
  ...
  scene.background = new THREE.Color(tok("--surface", "#f4f5f1"));
  ...
  function box(w: number, h: number, d: number, m: THREE.Material, x: number, y: number, z: number, round = 0, shadow = true) {
  ...
  box(20, 1500, 450, mat.ral7035glass, 0, 0, 0, 0, false);
  box(20, 1500, 450, mat.ral7035glass, 1580, 0, 0, 0, false);
  ...
  for (const x of [60, 1240]) box(24, 520, 40, mat.insul, x - 12, H - BAR_Y.L3 - 60, 120);
  PHASES.forEach((p, i) => {
    const y = H - BAR_Y[p];
    box(50, 1480 - y, 10, mat.bar, girisX((i + 1) as 1 | 2 | 3) - 25, y, 180);
  });
  box(40, 1480 - nY, 10, mat.bar, GIRIS_N_X - 20, nY, 185);
  staticLabels.push(label("Ana baralar 2 × 100×10 mm", 1400, H - BAR_Y.L3 - 80, 200));
  ...
  staticLabels.push(label("Termal dizi, kapağın içinde", 800, 1060, 440, "ours"));
  ...
  for (const pt of ALL_POINTS) {
    ...
    if (!pos) continue;
    const mesh = new THREE.Mesh(nodeGeo, new THREE.MeshStandardMaterial({ color: colors.stale, roughness: 0.45 }));
    mesh.position.set(pos.x, pos.y, pos.z);
    mesh.castShadow = true;
    const halo = new THREE.Mesh(haloGeo, new THREE.MeshBasicMaterial({ transparent: true, opacity: 0, depthWrite: false }));
    halo.visible = false;
  ...
  const detectors = DETECTOR_X.map((x, i) => {
    const id = `X2:${i + 1}`;
    const body = new THREE.Mesh(new THREE.CylinderGeometry(12, 12, 30, 16), new THREE.MeshStandardMaterial({ color: device, roughness: 0.6 }));
    body.position.set(x, 1125, 380);
    root.add(body);
    const cone = new THREE.Mesh(
      new THREE.ConeGeometry(330, 700, 32, 1, true),
      new THREE.MeshBasicMaterial({ color: ours, transparent: true, opacity: 0.07, side: THREE.DoubleSide, depthWrite: false }),
    );
  ...
  let dirty = true;
  let fly: { p0: THREE.Vector3; t0: THREE.Vector3; p1: THREE.Vector3; t1: THREE.Vector3; k: number } | null = null;
  const flyTo = (pos: THREE.Vector3, target: THREE.Vector3) => {
    fly = { p0: camera.position.clone(), t0: controls.target.clone(), p1: pos, t1: target, k: 0 };
  };
  ...
  const rank = (o: CSS2DObject) =>
    o === selectedLabel ? 0 : o.element.classList.contains("bad") ? 1 : o.element.classList.contains("ours") ? 2 : 3;
  function declutter() {
  ...
  const onMove = (e: PointerEvent) => {
    ...
    tip.textContent = node.point ? `${name}: ${num(node.point.t_c)} °C, ${STATE_TEXT[node.state]}` : `${name}: veri yok`;
  ...
  const api: SceneApi = {
    update(points, selected, tvoc, ackedPoints) {
      ...
      for (const [i, d] of detectors.entries()) {
        ...
        d.tag.element.textContent = d.broken ? `${d.id} arızalı, bu bölge korumasız` : d.id;
      }
      dirty = true;
    ...
    focusOn(pt) {
      ...
      flyTo(target.clone().addScaledVector(new THREE.Vector3(650, 380, 1900), fit()), target);
    },
    home() {
      flyTo(HOME_TARGET.clone().addScaledVector(HOME_OFFSET, fit()), HOME_TARGET.clone());
    },
    front() {
